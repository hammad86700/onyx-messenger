...
export default function MobileSettingsSheet({
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col h-full overflow-y-auto px-3.5 py-4 space-y-4 bg-[#07080b] chat-scroll-viewport">
      {/* Profile Card */}
      <div className="p-3.5 rounded-2xl bg-slate-900/70 border border-white/10 shadow-xl backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-2 mt-3">
          {onEditProfile && (
            <button
              ...
              className="py-2 px-3 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-semibold text-white border border-white/10 transition-colors flex items-center justify-center gap-1.5 touch-manipulation active:scale-95"
            >
          ...
          {onChangePassword && (
            <button
              ...
              className="py-2 px-3 rounded-xl bg-indigo-500/10 hover:bg-indigo-500/20 text-xs font-semibold text-indigo-300 border border-indigo-500/20 transition-colors flex items-center justify-center gap-1.5 touch-manipulation active:scale-95"
            >
          ...
          {THEMES.map((thm) => {
            ...
            return (
              <button
                ...
                className={`p-2.5 rounded-2xl border text-left transition-all touch-manipulation relative overflow-hidden ${
                  isSelected
              ...
              >
                ...
                <p className="text-xs font-bold">{thm.name}</p>
      ...
      <div className="p-3.5 rounded-2xl bg-gradient-to-tr from-amber-500/10 via-yellow-500/5 to-purple-500/10 border border-amber-500/30 relative overflow-hidden shadow-xl">
        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-8 h-8 rounded-xl bg-amber-500/20 border border-amber-500/30 text-amber-400 flex items-center justify-center shadow-md shrink-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-amber-200/80">Founder & Principal Architect</p>
            </div>
          </div>
          ...
          <a
            ...
            className="flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-pink-600 via-purple-600 to-amber-500 text-white text-[11px] font-bold shadow-md shadow-pink-500/20 active:scale-95 transition-transform touch-manipulation shrink-0 self-start sm:self-auto"
          >
      ...
      {onChangePassword && (
        <button
          onClick={onChangePassword}
          className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-gradient-to-r from-indigo-950/40 via-purple-950/30 to-slate-900 border border-indigo-500/30 text-white shadow-lg touch-manipulation active:scale-98 transition-all hover:border-indigo-500/60"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left overflow-hidden">
              <div className="flex items-center gap-1.5">
                <span className="text-xs font-bold text-white">Change Password</span>
                <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 font-semibold uppercase">
                  Security
                </span>
              </div>
              <p className="text-[11px] text-slate-400 truncate">Update your Onyx account credentials safely</p>
            </div>
          </div>
          <div className="p-2 rounded-xl bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 shrink-0">
            <ChevronRight className="w-4 h-4" />
          </div>
        </button>
      )}
      ...
      <button
        ...
        className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-gradient-to-r from-brand-900/30 via-indigo-950/40 to-slate-900 border border-brand-500/30 text-white shadow-lg touch-manipulation active:scale-98 transition-all hover:border-brand-500/60"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left overflow-hidden">
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-bold text-white">Install Onyx App</span>
              <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 font-semibold uppercase">PWA</span>
            </div>
            <p className="text-[11px] text-slate-400 truncate">Add to phone home screen like WhatsApp</p>
          </div>
        </div>
        <div className="p-2 rounded-xl bg-brand-600/30 text-brand-300 border border-brand-500/30 shrink-0">
      ...
      <button
        onClick={onLogout}
        className="w-full flex items-center justify-center gap-2 p-3.5 rounded-2xl bg-slate-900/90 hover:bg-slate-800 border border-white/10 text-slate-300 hover:text-white text-xs font-semibold transition-colors active:scale-98 touch-manipulation"
      >
